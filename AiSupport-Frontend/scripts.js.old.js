...
// ---- Lara chat (kept as-is) ----

const OLLAMA_URL = 'http://localhost:11434/api/generate'; // change if needed
const OLLAMA_MODEL = 'llama3.1:8b'; // ensure it's pulled
...
function initLara() {
  ...
  async function go() {
    ...
    try {
      await askOllama(q, (full) => { answer.textContent = full; });
      status.textContent = 'Klaar.';
    } catch (err) {
      status.textContent = '';
      answer.textContent = `⚠️ Kan geen verbinding maken met Ollama. Is het actief?\\n\\nFout: ${err.message}`;
    } finally {
      btn.disabled = false;
